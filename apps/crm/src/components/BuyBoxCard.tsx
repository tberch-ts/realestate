  ...
  return (
    <section className={`rounded-xl border ${banner.bg} p-5 mb-6`}>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-xs font-semibold tracking-wider text-gray-400">BUY BOX</h2>
          <p className={`text-2xl font-bold ${banner.text}`}>{banner.label}</p>
        </div>
        <div className="text-right flex gap-6">
          <div>
            <div className="text-xs text-gray-400">Market</div>
            <div className={`text-3xl font-bold ${scoreColor(buyBox.marketScore)}`}>{buyBox.marketScore}</div>
            <div className="text-[10px] text-gray-500">public data only</div>
          </div>
          <div>
            <div className="text-xs text-gray-400">Full</div>
            <div className={`text-3xl font-bold ${scoreColor(buyBox.fullScore)}`}>{buyBox.fullScore}</div>
            <div className="text-[10px] text-gray-500">with deal form</div>
          </div>
        </div>
      </div>
          ...
          <li key={c.criterion} className="flex items-start gap-2 text-sm">
            ...
            <div className="flex-1">
              <div className="flex justify-between">
                ...
                <span className="text-gray-400 text-xs">{c.actual ?? '—'}</span>
              </div>

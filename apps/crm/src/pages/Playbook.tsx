...
// deal board, etc.) so the guide doubles as a navigation shortcut.
...
const STAGES: Stage[] = [
  {
    ...
    cta: { label: 'Open the hotspot map', to: '/app/hotspots' },
  },
  {
    n: 2,
    ...
    cta: { label: 'Pull property data', to: '/app/property-search' },
  },
  {
    n: 3,
    title: 'Tour & verify',
  ...
  },
  {
    ...
    cta: { label: 'Generate an LOI', to: '/app/loi' },
  },
  {
    n: 5,
  ...
  },
  {
    ...
    cta: { label: 'Track a capital raise', to: '/app/capital' },
  },
  {
    n: 8,
    title: 'Closing',
  ...
  },
  {
    ...
    cta: { label: 'Track take-over follow-ups', to: '/app/followup' },
  },
  {
    n: 10,
    title: 'Execute & exit',
...
export default function Playbook() {
  return (
    <div>
      <h1 className="text-2xl font-bold mb-3">Deal Playbook</h1>
      <p className="text-sm text-gray-500 mb-8 max-w-2xl">
        You underwrote a property. What now? This is the field guide — 10 stages from first tour
        through exit, with tips, checklists, and the watch-outs that bite first-time sponsors.
      </p>

      <div className="mb-8 p-4 rounded-xl border" style={{ borderColor: 'var(--border)', background: 'var(--bg-card)' }}>
        <div className="text-xs font-semibold uppercase tracking-wide text-gray-400 mb-3">Jump to stage</div>
      ...
      <div className="mt-10 p-5 rounded-xl border border-blue-500/30 bg-blue-500/5">
        <h2 className="text-lg font-semibold mb-2">Ready for the next deal?</h2>
        <p className="text-sm text-gray-400 mb-4">
          The best time to source the next deal is right after closing the current one — your
          underwriting muscles are warm and your network just saw you close.
        </p>
        <div className="flex flex-wrap gap-3">
          <Link to="/app/property-search" className="text-sm px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 transition-colors font-semibold">
            Analyze a new address
          </Link>
          <Link to="/app/hotspots" className="text-sm px-4 py-2 rounded-lg border hover:bg-white/5 transition-colors" style={{ borderColor: 'var(--border)' }}>
            Browse hot zones
          </Link>
  ...
  return (
    <section id={`stage-${stage.n}`} className="p-5 rounded-xl border scroll-mt-8" style={{ borderColor: 'var(--border)', background: 'var(--bg-card)' }}>
      ...
      <div className="pl-14">
        ...
        {stage.cta && (
          <Link to={stage.cta.to} className="inline-block text-sm text-blue-400 hover:text-blue-300 font-semibold">
            {stage.cta.label} →
          </Link>
        )}
      </div>

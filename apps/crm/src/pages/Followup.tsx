    ...
    <div>
      ...
      <div className="flex flex-wrap gap-3 mb-4 items-end">
        <Filter label="Min units">
          <input type="number" value={minUnits} onChange={(e) => setMinUnits(Number(e.target.value) || 0)} className={INPUT} style={bd} />
        </Filter>
        <Filter label="Min year built">
          <input type="number" value={minYear} onChange={(e) => setMinYear(Number(e.target.value) || 0)} className={INPUT} style={bd} />
        </Filter>
        {result && (
      ...
      {result && rows.length === 0 && !loading && (
        <div className="p-4 rounded border text-gray-400 text-sm" style={{ borderColor: 'var(--border)' }}>
          No candidates in this zone with units ≥ {minUnits} and year built ≥ {minYear}. Try loosening the filters.
        </div>
      )}
    </div>
  );
...
function Row({ c }: { c: FollowupScored }) {
  return (
    <tr className="border-t hover:bg-white/5 transition-colors" style={{ borderColor: 'var(--border)' }}>
      ...
      <td className="py-2 px-3 text-right text-gray-200 align-top">{c.units ?? '—'}</td>
      <td className="py-2 px-3 text-right text-gray-300 align-top">{c.yearBuilt ?? '—'}</td>
      ...
      <td className="py-2 px-3 text-right text-gray-300 align-top">
        ...
        {c.saleDate && <div className="text-xs text-gray-500">{c.saleDate}</div>}
      </td>
...
function OwnerBadge({
  ...
  return (
    <div className="flex flex-wrap gap-1 mt-1">
      ...
      {outOfState && state && (
        <span className="text-[10px] px-1.5 py-0.5 rounded border border-amber-500/40 bg-amber-500/10 text-amber-200">OOS: {state}</span>
      )}
    </div>
  );
}
...
const INPUT = 'rounded-lg border px-3 py-1.5 text-sm bg-transparent w-24';
